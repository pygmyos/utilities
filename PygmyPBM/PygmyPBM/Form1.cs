...
namespace WindowsFormsApplication1
{
    ...
    public partial class Form1 : Form
    {
        ...
        const int PYGMY_VECTOR_MASK                 = 0xE0;//(BIT7|BIT6|BIT5)
        const int PYGMY_VECTOR_END                  = 0x00;//
        const int PYGMY_VECTOR_COLOR                = 0x20;//BIT5
        const int PYGMY_VECTOR_POLY                 = 0x40;//BIT6
        const int PYGMY_VECTOR_ARC                  = 0x60;//(BIT6|BIT5)
        const int PYGMY_VECTOR_SPLINE               = 0x80;//BIT7
        const int PYGMY_VECTOR_TEXT                 = 0xA0;//(BIT7|BIT5)
        const int PYGMY_VECTOR_RASTER               = 0xC0;//(BIT7|BIT6)
        ...
        int guiGetEntries( )
        {
            ...
            if ( (iPygmyInfo & (PYGMY_PBM_IMAGESTRIP | PYGMY_PBM_FONT)) == 0)
            {
                ImageFrames = 0;
                return (0);
            } 
        ...
        int guiGetImage( int uiIndex)
        {
            ...
            if ((iPygmyInfo & PYGMY_PBM_TABLE32) != 0)
            {				
        ...
        public void LoadImage( string FileName, int iIndex )
        {
            ...
            else
            {
                ...
                if ( (iPygmyInfo & PYGMY_PBM_16BITD) != 0)
                ...
                if ((iPygmyInfo & PYGMY_PBM_IMAGE) != 0)
                {
                    treeView1.Nodes.Add("Type: Image");
                }
                else if ((iPygmyInfo & PYGMY_PBM_IMAGESTRIP) != 0)
                {
                    treeView1.Nodes.Add("Type: ImageStrip");
                }
                else if ((iPygmyInfo & PYGMY_PBM_FONT) != 0)
                {
                    treeView1.Nodes.Add("Type: Font");
                }
                else if ((iPygmyInfo & PYGMY_PBM_VECTOR) != 0)
                {
                ...
                for (iPixelIndex = 0; iPixelIndex < iLen && ( imageFile.BaseStream.Position < imageFile.BaseStream.Length ) ; )
                {
                    ...
                    if (iBPP == PYGMY_PBM_1BPP && (iPacket & PYGMY_PACKET_RLE) == 0)
                    {
                        iCount = 7;
                    } // if
                    ...
                    {
                        if (i == 0 || (iPacket & PYGMY_PACKET_RLE) == 0)
                        {
                            if (iBPP == PYGMY_PBM_1BPP)
                            {
                                if ((iPacket & PYGMY_PACKET_RLE) == 0)
                                {
                                    if ((iPacket & (PYGMY_PACKET_SET >> i)) != 0)
                                    {
                                        iPacket |= PYGMY_PACKET_SET;
                                    }
                                    else
                                    {
                                        iPacket &= ~PYGMY_PACKET_SET;
                        ...
                        if ( (iPacket & PYGMY_PACKET_SET) != 0)
                        {
                            if ((iPygmyInfo & PYGMY_PBM_FONT) != 0 && (iPygmyInfo & PYGMY_PBM_1BPP) != 0)
                            {
                                Pixel = Color.FromArgb(0xFF, 0xFF, 0xFF);
                            }
                            else if (iBPP != PYGMY_PBM_1BPP)
                            {
                        ...
                        else if ((iPygmyInfo & PYGMY_PBM_ALPHA) == 0)
                        {
                            if ((iPygmyInfo & PYGMY_PBM_FONT) != 0 && (iPygmyInfo & PYGMY_PBM_1BPP) != 0)
                            {
        ...
        public void exportPBM( string FileName, int uiBPP )
        {
            int[] ucBitMask = { 0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01 };
            int i, ii, ulLen;
            int uiPBMHeader, uiPixelRep, uiRawPixels;
            Color[] tmpPixels;
            ...
            toolStripStatusLabel1.Text = uiBPP.ToString();
            if (uiBPP == 1)
            {
                uiBPP = PYGMY_PBM_1BPP;
            }
            else if (uiBPP == 4)
            {
                uiBPP = PYGMY_PBM_4BPP;
            }
            else if (uiBPP == 8)
            {
                uiBPP = PYGMY_PBM_8BPP;
            }
            else if (uiBPP == 12)
            {
                uiBPP = PYGMY_PBM_12BPP;
            }
            else if (uiBPP == 16)
            {
                uiBPP = PYGMY_PBM_16BPP;
            }
            else if (uiBPP == 24)
            {
                uiBPP = PYGMY_PBM_24BPP;
            }
            /*if (ImageWidth > 255 || ImageHeight > 255)
            {
                uiPBMHeader = (uiBPP | PYGMY_PBM_16BITD);
            }
            else
            {*/
                uiPBMHeader = (uiBPP);
            //}
            bw.Write( (byte)(uiPBMHeader >> 8) );
            bw.Write( (byte)uiPBMHeader );
            /*if (ImageWidth > 255 || ImageHeight > 255)
            {
            ...
            for( i = 0; i < ulLen; ) 
            {
                ...
                if( uiBPP == PYGMY_PBM_1BPP )
                {
                    if( uiPixelRep > 7 ) 
                    {
                        ...
                        {
                            bw.Write((byte)((uiPixelRep-1) | PYGMY_PBM_RLE | PYGMY_PBM_PIXELON)); // BIT7 set for RLE, BIT6 set for pixel ON
                        } 
                        else
                        {
                            bw.Write((byte)((uiPixelRep-1) | PYGMY_PBM_RLE )); //BIT7 set for RLE ON, BIT6 clear for pixel off/transparent
                        } 
                    } 
                    else 
                    {
                ...
                else if( uiBPP == PYGMY_PBM_4BPP )
                {
                    if( uiPixelRep > 0 )
                    {
                        bw.Write((byte)(uiPixelRep-1) | PYGMY_PBM_RLE | PYGMY_PBM_PIXELON);
                ...
                else if( uiBPP == PYGMY_PBM_16BPP ) 
                {
                    if( uiPixelRep == 0   )
                    {
                        bw.Write((byte)PYGMY_PBM_PIXELON );
        ...
        {
            if (ImageName != null && comboBoxFrames.SelectedIndex < ImageFrames)
            {
